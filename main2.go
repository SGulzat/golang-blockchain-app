...
func main() {
    ...
    log.Fatalf("parsedABI.Pack() metgod: %v", data)
    ...
    var gasPrice *big.Int = big.NewInt(1000000000) // 1 Gwei в wei

    // func NewTransaction(nonce uint64, to common.Address, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte) *Transaction
    tx := types.NewTransaction(nonce, contractAddressHex, value, gas, gasPrice, nil)
...
func assignTaskToContract(devs []Developer, task Task) (int64, error) {
	client, err := ethclient.Dial("https://mainnet.infura.io/v3/YOUR_INFURA_PROJECT_ID")
	if err != nil {
		log.Fatalf("Не удалось подключиться к сети: %v", err)
	}

	contractAddress := common.HexToAddress("0xYourContractAddress")
	parsedABI, err := abi.JSON(strings.NewReader(string(contractInfo.ABI)))
	if err != nil {
		return 0, fmt.Errorf("Не удалось распарсить ABI: %v", err)
	}

	// Пример вызова метода assignTask смарт-контракта
	auth, err := bind.NewKeyedTransactorWithChainID(privateKey, big.NewInt(1)) // ChainID для mainnet - 1
	if err != nil {
		return 0, fmt.Errorf("Не удалось создать объект для подписи: %v", err)
	}

	taskLevel := task.Level // Уровень задачи (например, "Easy", "Medium", "Hard")

	// Вызов функции assignTask в смарт-контракте
	tx, err := parsedABI.Pack("assignTask", big.NewInt(int64(task.ID)), taskLevel)
	if err != nil {
		return 0, fmt.Errorf("Не удалось упаковать данные для assignTask: %v", err)
	}

	gasLimit := uint64(300000) // Установите лимит газа
	gasPrice, err := client.SuggestGasPrice(context.Background())
	if err != nil {
		return 0, fmt.Errorf("Не удалось получить цену за газ: %v", err)
	}
	...
	tx = types.NewTransaction(nonce, contractAddress, big.NewInt(0), gasLimit, gasPrice, tx)

	signedTx, err := auth.Signer(auth.From, tx)
	if err != nil {
		return 0, fmt.Errorf("Не удалось подписать транзакцию: %v", err)
	}

	err = client.SendTransaction(context.Background(), signedTx)
	if err != nil {
		return 0, fmt.Errorf("Не удалось отправить транзакцию: %v", err)
	}

	fmt.Printf("Транзакция отправлена с hash: %s\n", signedTx.Hash().Hex())

	// Получаем результат выполнения assignTask
	receipt, err := bind.WaitMined(context.Background(), client, signedTx)
	if err != nil {
		return 0, fmt.Errorf("Не удалось дождаться завершения транзакции: %v", err)
	}

	if receipt.Status != types.ReceiptStatusSuccessful {
		return 0, fmt.Errorf("Транзакция не удалась")
	}

	// Получаем ID назначенного разработчика из смарт-контракта
	assignedDevId := contract.GetAssignedDeveloper(nil, big.NewInt(int64(task.ID)))
	return assignedDevId.Uint64(), nil
}
